...
const createUser = async function (data) {
    let sql = `
        INSERT INTO users(userName, u_type, u_password, firstname, lastname, adress, postcode, city, saldo)
        VALUES('${data.username}', '${data.u_type}', '${data.u_password}', '${data.firstname}', '${data.lastname}', '${data.adress}', '${data.postcode}', '${data.city}', ${data.saldo});`;
    const results = await db.connection.promise().query(sql)
